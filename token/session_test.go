...
func TestSignedCookieStructure(t *testing.T) {
    ...
    decoded, _ := base64.RawURLEncoding.DecodeString(cookieVal)
    if len(decoded) != SIGNATURE_LEN + len(sessKey) {
        t.Errorf("Expected length %d, got %d", SIGNATURE_LEN + len(sessKey), len(decoded))
    }
}
