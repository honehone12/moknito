...
func (s *SessionTokenSigner) SignedCookie(sessKey []byte, nonce string) (string, error) {
	...
	l := len(sessKey)
	raw := make([]byte, HMAC_SIGNATURE_LEN+l)
	if n := copy(raw[:HMAC_SIGNATURE_LEN], hash); n != HMAC_SIGNATURE_LEN {
		return "", errors.New("failed to copy signature")
	}
	if n := copy(raw[HMAC_SIGNATURE_LEN:], sessKey); n != l {
